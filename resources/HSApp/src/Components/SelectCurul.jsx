...
const SelectCurul = ({handlerCurulSelected, esVotacion=false, corporacion = 1, curulSelectable = true, linkToClickCurul= "#", curules = [{id: 0, cx: 0, cy: 0, r: 0, seccionAsiento: "", filaAsiento: "", asiento: 0, activo: true}], showButtonProfile= false, curulIdSelected = 0, curulName = "curul-1", origen = "", minimized = true, readOnly = false, filters = [], handlersForEachFilter = [], handlerQuickSectionSelector = null, handlerQuickRowSelector = null, handlerQuickCurulSelector = null}) => {
    if(curules.length === 0){
                ...
                <div className="curul" onMouseDown={(e)=>{mouseDownHandler(e, curulName)}}>
                    <svg data-zoom={3} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 720 776">
                        ...
                        <g id="PeopleDots">
                            ...
                            {
                                curules.map((x,i) => {
                                    let colorClaseVotacion = "";
                                    if(esVotacion && x.congresista !== null){
                                        switch (x.congresista.tipo_respuesta_votacion_id) {
                                            case 1:
                                                colorClaseVotacion = "afavor";
                                            break;
                                            case 2:
                                                colorClaseVotacion = "encontra";
                                            break;
                                            case 3:
                                                colorClaseVotacion = "ausente";
                                            break;
                                        ...
                                        <g className="curulItem">
                                            ...
                                            onClick={(e)=>{
                                                if(x.congresista === null && curulSelectable){
                                                    ...
                                                    window.open(`#/perfil-congresista/${x.congresista.id}`, '_blank');
                                                }
                                            }}
                                            key={i} style={
                                                {fill: x.congresista !== null ? x.congresista.partido.color : "", stroke: x.congresista !== null ? x.congresista.partido.color : ""}
                                            } curul_id={x.id} className={`cls-2 ${x.id === curulIdSelected ? "declared" : ""} ${esVotacion ? colorClaseVotacion : ""}`} cx={Number(x.cx)} cy={Number(x.cy)} r={x.r} seccionAsiento={x.seccionAsiento} filaAsiento={x.filaAsiento} asiento={x.asiento} activo={x.activo} />
                                        </g>
                                    )
                                })
                            }
